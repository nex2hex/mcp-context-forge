...
async def get_current_user_with_permissions(
    ...
    This function no longer holds a database session for the entire request lifecycle.
    Permission decorators use fresh_db_session() for short-lived permission checks,
    preventing "idle in transaction" connection accumulation under high load.
...
def require_permission(permission: str, resource_type: Optional[str] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            """
            # Extract user context from kwargs (no longer requires "db" key)
            user_context = None
            ...
            # Use fresh_db_session() for short-lived permission check to avoid holding
            # connections during slow downstream operations (Issue #1865)
            with fresh_db_session() as perm_db:
                permission_service = PermissionService(perm_db)
...
def require_admin_permission():
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            # Use fresh_db_session() for short-lived permission check (Issue #1865)
            with fresh_db_session() as perm_db:
                permission_service = PermissionService(perm_db)
...
class PermissionChecker:
    ...
    Uses fresh_db_session() for each permission check to avoid holding connections
    during slow downstream operations (Issue #1865).
